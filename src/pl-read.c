...
struct var_t { char   *name;
	       int     times;
	       cell_t *ref;
	       Var     next;
	     };

typedef struct { tok_type  type;
		 // long start;
		 // long end;
                 union { atom_t   atom;
                         long     intg;
    ...
    { switch(c)
      { case '*': if ((c=Getc(S))=='/')
        ...
        case '/': if ((c=Getc(S))=='*')
                  ...
                  continue;
        case EOF: SyntaxError("Unexpected EOF in bracketed comment");
                  return(0);
      }
      c=Getc(S);
    }
  ...
  { c=Getc(S);
    ...
    { switch(c)
      { case '*': if ((c=Getc(S))=='/')
                    return(1);
                  continue;
        case EOF: SyntaxError("Unexpected EOF");
                  return(0);
      }
      c=Getc(S);
    }
...
{ Var v=varl_first;
  ...
  for (;;v=v->next)
  { if (v->name) free(v->name);	// FIXME : suppose that name was malloced
    if (v==varl_tail)
      break;
  }
  ...
  { case '`':
    ...
    case '\\': switch (c=Getc(S))
               { case 'a':  c='\a'; break;
                 ...
                 case '5':
                 case '6':
                 case '7':  c=oct_escape(S, c);
                            break;
                 default:   UnGetc(c); c='\\'; break;
                 // default:   break;	// FIXME : non-ISO
               }
               break;
    case EOF:  SyntaxError("Unexpected EOF in quoted string");
    	       UnGetc(c);
               return(-1);
    default:   break;
  }
...
{ unsigned long val=0;
  ...
  if (c=='0')
  { switch(c=Getc(S))
    { case '\'':  if ((val=read_quoted_char(S))>=0)
                    goto intg;
		  else
		    goto error;
      case 'b':   ISO_NUM(S,isBinDigit,2);
      case 'o':   ISO_NUM(S,isOctDigit,8);
      case 'x':   ISO_NUM(S,isHexDigit,16);
      case_default:
      default:    UnGetc(c);
		  c='0';
		  break;
...
{ static atom_t atom;
  ...
  { case EOF:  if (seeingString(S))
    ...
    case '5':  case '6': case '7': case '8': case '9':
	       ...
	       { return(0); }
    case '!':
    case ';':  atom=PL_char_to_atom(c);
               goto case_atom;
    case '%':  strip_PL_comment(S);
               c=' ';
	       goto loop;
    case '[':  SkipSpaces(S,c);
               if (c==']')
...
{ node_t elem;
  ...
  if (!read_term(S,1200,",)",&elem)) return(0);
  else level++;
...
loop:
  ...
  switch(token.type)
  { case ']': addr[2].celp=new_atom(ATOM(nil));
	      unget_token=0;
              return(1);
    case '|': get_token(S);
              if (!read_term(S,1200,"]",&elem) || token.type!=']') return(0);
              addr[2]=elem.cell;
	      unget_token=0;
              return(1);
    case ',': get_token(S);
      ...
      if (PL_is_op(OP_INFIX,atom,&type,&prec))
        { if (max>=prec)
            ...
            { case OP_XFX: if (node_l.prec <  prec)
                             { m=prec-1; goto infix; }
                           break;
              case OP_XFY: if (node_l.prec <  prec)
                             { m=prec; goto infix; }
		           break;
              case OP_YFX: if (node_l.prec <= prec && max > prec)
                             { m=prec-1; goto infix; }
                           break;
              case OP_YFY: if (node_l.prec <= prec && max > prec)
                             { m=prec; goto infix; }
		           break;
              infix:     { node_t node_r;
      ...
      if (PL_is_op(OP_POSTFIX,atom,&type,&prec))
        { if (max>=prec)
            ...
            { case OP_XF: if (node_l.prec <  prec)
                            goto postfix;
                          break;
              case OP_YF: if (node_l.prec <= prec && max > prec)
                            goto postfix;
                          break;
...
{ node_t node_s;
  ...
  switch(tok)
  { case '(':      if (read_term(S,1200,")",&node_s) &&  // '(' '('
                       token.type==')' )
                   { unget_token=0;
		     goto case_simple;
		   }
    ...
    case '{':      if (read_term(S,1200,"}",&node_s) &&	// '{'
		     ...
		     goto case_simple;
                     ...
                     PL_warning("read_term : error in '{' term '}'");
    case '}':      goto case_term_a;
    case '[':      if (!read_list(S, &node_s))
                     PL_warning("read_term : error in '[' ... ']'");
		   else
		     goto case_simple;
    case T_VOID:   node_s.cell.celp=token.tok_val.ref;
		   goto case_simple;
    case T_VAR:    node_s.cell.celp=token.tok_val.var->ref;
		   goto case_simple;
    case T_INTG:   node_s.cell.val=__intg(token.tok_val.intg);
		   goto case_simple;
    case T_FLT:    node_s.cell.celp=new_flt(token.tok_val.flt);
		   goto case_simple;
    case T_ATOM:   node_s.cell.val=__atom(token.tok_val.atom);
		   goto case_simple;
    case T_STRING: node_s.cell.celp=token.tok_val.ref;
		   goto case_simple;
    case ']':
    case ',':
    case '|':
