	...
	if (PL_get_atom_chars(full, &f)) {
		...
		if (*s == '.') {
			if (PL_get_atom_chars(ext, &e)) {
				...
				if (PL_PathCmp(e, s + 1))
					fail;
			} else if (!PL_unify_atom_chars(ext, s + 1))
				fail;
		...
		if (PL_unify_atom(ext, ATOM(_)) && pl_unify(full, base))
			succeed;
		fail;
	} else if (PL_get_atom_chars(base, &b) && PL_get_atom_chars(ext, &e)) {
		char *s;
		if (e[0] == '.')
			e++;
...
int pl_access_file(term_t name, term_t mode)
{
	...
	if (m == ATOM(_write) || m == ATOM(_append))
		md = PL_ACCESS_WRITE;
	else if (m == ATOM(_read))
		md = PL_ACCESS_READ;
	else if (m == ATOM(_execute))
		md = PL_ACCESS_EXECUTE;
	else if (m == ATOM(_exist))
		md = PL_ACCESS_EXIST;
	else
