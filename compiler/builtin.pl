...
%% inline(unground(X)):+ +> comm(unground,X),
%% 		   +> g('{ cell_t *c;'),
%% 		   +> new_indent(2),
%% 		   code_Assign(c,X),
%% 		   +> old_indent,
...
inline(succ(X,Y)):+ +> comm(succ,X,Y),
		   ...
		   +> old_indent,
		   +> g('  Deref(d0);'),
		   +> g('  if (get_tag(d0)==intg_tag)'),
		   +> g('    { if (!PL_unify_intg(d1,get_val(d0)+1))'),
		   +> g('        goto backtrack;'),
		   +> g('    }'),
		   +> g('  else'),
		   +> g('  { Deref(d1);'),
		   +> g('    if (get_tag(d1)==intg_tag)'),
...
%% 		   +> g('}').
%% 
%% inline(write_ln(X)):+ +> comm(write_ln,X),
%% 		   +> g('{ cell_t *c;'),
%% 		   +> new_indent(2),
%% 		   code_Assign(c,X),
%% 		   +> old_indent,
