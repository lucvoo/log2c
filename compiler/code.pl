...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
code_Assign(V,E):+ code_X(E,X), +> assign(X,V), !.
code_Assign(V,E):+ E=fun(F,Nf,A),
		   mapli(0,code_U,A,L),
		   +> assign(struct(F,Nf,L),V).

code_AssignD(V,E):+ code_X(E,X), +> assignD(X,V), !.
code_AssignD(V,E):+ E=fun(F,Nf,A),
		   mapli(0,code_U,A,L),
...
code_M(V,E)	:+ get_lv(O, Nv,E,_), flag(rho,L,L),
		   ( O==f  -> warning('unbound var in math');
		     ...
		     O==v  -> warning('void var in math');
		     O==rt -> +> g('GETINTG(TMP_~d,~w);',[Nv,V]);
		     O==r  -> I is L+Nv, +> g('GETINTG(FP[~d].celp,~w);',[I,V]) ).
code_M(V,E)	:+ E=fun(F,2,[A,B]),
		   map_arith_op(F/2,Op),
		   gensym('i_',J),
		   +> g('{ int ~w;',[J]),
		   +> new_indent(2),
...
code_M(V,E)	:+ E=fun(F,1,[A]),
		   ...
		   +> g('~w=~w(~w);',[V,Op,V]).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
code_U(I,E)	:+ code_X(E,X), +> u(X,I), !.
code_U(I,E)	:+ E=fun(F,N,A),
		   mapli(0,code_U,A,L),
		   +> u(struct(F,N,L),I).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

code_UA(I,E)	:+ code_X(E,X), +> get_(I,X), !.
code_UA(I,E)	:+ E=fun(F,Nf,A),
		   mapli(0,code_U,A,L),
		   +> get_(I,struct(F,Nf,L)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
code_Arg(I,E)	:+ code_X(E,X), +> load_(I,X), !.
code_Arg(I,E)	:+ E=fun(F,Nf,A),
		   mapli(0,code_U,A,L),
